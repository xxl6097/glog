...
func (f *LoggerFactory) Create() *zap.Logger {
...
func (f *LoggerFactory) createProductionLogger() *zap.Logger {
	level := zap.NewAtomicLevel() //zap.InfoLevel
...
func (f *LoggerFactory) createDevelopmentLogger() *zap.Logger {
...
func (f *LoggerFactory) createDevelopmentLogger1() *zap.Logger {
	config := zap.NewDevelopmentEncoderConfig()
	config.EncodeLevel = zapcore.CapitalColorLevelEncoder
	config.EncodeTime = func(t time.Time, enc zapcore.PrimitiveArrayEncoder) {
		enc.AppendString(t.Format("2006-01-02 15:04:05.000"))
